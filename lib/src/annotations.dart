...
import 'package:borsh_annotation/borsh_annotation.dart' as borsh_annotation show BFixedArray ;
...
class BU128 extends BType<BigInt> {
  ...
  void write(BinaryWriter writer, BigInt value) {
    ExtendedBinaryWriter extendedWriter = ExtendedBinaryWriter.fromBinaryWriter(writer);
    extendedWriter.writeU128(value);
  }

  @override
  BigInt read(BinaryReader reader) {
    ExtendedBinaryReader extendedReader = ExtendedBinaryReader.fromBinaryReader(reader);
...
class BFixedArray<T> extends borsh_annotation.BFixedArray<T>{
  ...
  void write(BinaryWriter writer, List<T> value) {
    if (value.length != length) {
      throw ArgumentError('Expected List of length $length, got ${value.length}');
    }
